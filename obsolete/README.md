...
2. **`DP-5` disappears from mutter's topology entirely.** `gdctl show` lists
   ...
   **Side effect observed 2026-09-14: can black out the BenQ too, not just
   the Samsung.** `journalctl` for the incident window showed `gnome-shell`
   hitting repeated `drmModeAtomicCommit: Invalid argument`, `Failed to post
   KMS update`, and `Failed to create KMS output: No modes available` at the
   exact moment `DP-5` dropped, plus `logical_monitor != NULL` assertion
   failures. Since `DP-5` is MST sub-topology riding the same physical DP
   link/atomic-commit transaction as `DP-2`, a commit that fails because
   `DP-5`'s connector state just went invalid can stall the *whole* KMS
   pipeline, not only the dropped branch — so the BenQ can go blank too,
   and the system can feel fully unusable, until a later commit succeeds
   (e.g. once the Samsung is power-cycled and `revive-samsung.sh` runs). As
   before, no kernel/nvidia messages appear in `dmesg`/`journalctl -k` for
   any of this — it's entirely inside the closed-source driver. Toggling the
   BenQ's own power does nothing in this state, since the BenQ isn't the
